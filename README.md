...
Щоб карма за реакції нараховувалася коректно, бот повинен бути правильно створений, налаштований та доданий у групу:

### 1. Створення бота у BotFather
1. Знайдіть у Telegram офіційного бота **@BotFather** та почніть діалог.
2. Надішліть команду `/newbot` та слідуйте інструкціям (введіть ім'я та юзернейм бота, що має закінчуватися на `bot`).
3. Скопіюйте отриманий **API Token** (напр. `123456789:ABCdef...`).

### 2. Вимкнення режиму приватності (Group Privacy) — КРИТИЧНО!
За замовчуванням боти бачать лише команди, що починаються з `/`. Щоб бот міг реєструвати повідомлення учасників спільноти (і нараховувати за них карму), режим приватності **має бути вимкнено**:
1. У чаті з **@BotFather** надішліть команду `/setprivacy`.
2. Оберіть вашого бота з меню.
3. Натисніть кнопку **Disable** (має з'явитися повідомлення: `Success! New status: DISABLED`).
...
### 4. Налаштування в Адмін-панелі сервісу
1. Перейдіть в адмін-панель за адресою `https://your-domain.top/admin`.
2. Вкажіть наступні обов'язкові параметри у вкладці **Налаштування**:
   * **Telegram Bot Token**: Токен, який ви отримали від @BotFather.
   * **Chat ID**: Числовий ID вашої групи (зазвичай починається з `-100`, наприклад `-100123456789`).
   * **WebApp URL**: Повна URL-адреса вашого сайту (наприклад, `https://top-50.srvrs.top/`), яка буде прив'язана до кнопки відкриття Mini App у Telegram.
3. Збережіть налаштування. Сервіс автоматично перезапустить бота для застосування конфігурації.
