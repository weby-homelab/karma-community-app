...
*   **Почесне місце власника чату (Honorary Owner Placement):** Можливість відобразити власника чату/групи окремо від загальної таблиці рейтингу з почесною короною 👑.
*   **Динамічна дата оновлення (Dynamic Update Status):** Автоматичний розрахунок та відображення часу останнього імпорту даних у футері сторінки.
*   **Розширений імпорт (Channel & User Support):** Повна підтримка повідомлень від імені користувачів та каналів (ідентифікатори `user` та `channel` з JSON Telegram Desktop).
